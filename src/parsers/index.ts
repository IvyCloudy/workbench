 ...
 *    2. ensureTrackingColumns：为表格补全 testcase_id 列（推送唯一主键）。
 *    3. applyTestCaseNos：推送成功后按 testcase_id 回写 testCaseNo 列。
 *    4. parseFileToRows：资源管理器右键推送路径使用。
 *  设计要点：
 *    - testcase_id 只可从本文件生成，避免多处独立实现造成 ID 不一致。
 *    - YAML/JSON 同时将 testcase_id/testCaseNo 写入 sourceData，以保证 save 重建嵌套结构时不丢字段。
 * ============================================================================
 */
...
export function ensureTrackingColumns(
    ...
    // 同步把 testcase_id 写回 sourceData（YAML/JSON 保存时会优先用 sourceData 重建嵌套结构）
    if (sourceData) {
...
/**
 * 推送成功后，按 testcase_id 把 testCaseNo 回写到 tableData / sourceData。
 *   - 若 testCaseNo 列不存在，则插入到 testcase_id 列右侧
...
export async function parseFileToRows(filePath: string): Promise<any[] | null> {
    ...
    // CSV: 直接由 headers + rows 组装对象（已包含 testcase_id 列）
    if (type === 'csv') {
    ...
    // YAML / JSON: 优先用 sourceData（保留嵌套结构），并把 testcase_id 注入到每条记录上
