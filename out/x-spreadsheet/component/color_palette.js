...
function buildTd(bgcolor) {
    return (0, element_1.h)('td', '').child((0, element_1.h)('div', `${config_1.cssPrefix}-color-palette-cell`)
        .on('click.stop', () => this.change(bgcolor))
        .css('background-color', bgcolor));
}
class ColorPalette {
    constructor() {
        this.el = (0, element_1.h)('div', `${config_1.cssPrefix}-color-palette`);
        this.change = () => { };
        const table = (0, element_1.h)('table', '').children((0, element_1.h)('tbody', '').children((0, element_1.h)('tr', `${config_1.cssPrefix}-theme-color-placeholders`).children(...themeColorPlaceHolders.map(color => buildTd.call(this, color))), ...themeColors.map(it => (0, element_1.h)('tr', `${config_1.cssPrefix}-theme-colors`).children(...it.map(color => buildTd.call(this, color)))), (0, element_1.h)('tr', `${config_1.cssPrefix}-standard-colors`).children(...standardColors.map(color => buildTd.call(this, color)))));
        this.el.child(table);
    }
}
