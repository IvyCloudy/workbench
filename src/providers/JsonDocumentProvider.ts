...
export function isQualifiedJsonFile(uri: vscode.Uri): boolean {
    if (uri.scheme !== 'file' || !/\.json$/i.test(uri.fsPath)) {
        return false;
    }
    ...
    return (rootDir === '测试任务' || rootDir === 'testtask') &&
           ...
           /\.json$/i.test(dirNames[len - 1]);
}

// 解析JSON文件数据（支持复杂嵌套结构）
function parseJsonData(filePath: string): { headers: string[], rows: string[][] } {
    try {
...
export class JsonEditorProvider implements vscode.CustomEditorProvider {
    ...
    async resolveCustomEditor(document: vscode.CustomDocument, webviewPanel: vscode.WebviewPanel, _token: vscode.CancellationToken): Promise<void> {
        ...
        const fileName = filePath.split(path.sep).pop() || 'JSON';
        webviewPanel.title = fileName + ' - 测试案例编辑器';

        webviewPanel.webview.options = { enableScripts: true, localResourceRoots: [this.extensionUri] };

        if (!isQualifiedJsonFile(document.uri)) {
            webviewPanel.webview.html = this.getErrorHtml('该文件不在允许的目录下，仅支持：<br>测试任务/xxx/输入文档/测试案例/*.json');
            return;
        }
        ...
        webviewPanel.webview.onDidReceiveMessage(async (msg: any) => {
            if (msg?.type === 'init') {
                const freshData = parseJsonData(filePath);
                const dataStr = JSON.stringify(freshData);
                const encoder = new TextEncoder();
                const uint8Array = encoder.encode(dataStr);
                webviewPanel.webview.postMessage({ type: 'jsonData', data: Array.from(uint8Array) });
            }
            ...
            if (msg?.type === 'pushTestCase' && msg?.data) {
                console.log('[JSON推送] 收到推送请求');
                console.log('[JSON推送] 文件路径:', filePath);
                console.log('[JSON推送] 数据:', JSON.stringify(msg.data, null, 2));
                try {
                    ...
                    console.log('[JSON推送] 路径部分:', parts);
                    const testTaskNo = parts.find((p, i) =>
                        p.startsWith('TT') || /^\d+$/.test(p.slice(0, 2))
                    ) || '';
                    console.log('[JSON推送] testTaskNo:', testTaskNo);
                    const result = await queryApi({
                        testTaskNo: testTaskNo,
                        currentPage: 1,
                        pageSize: 10
                    }, ctx);
                    console.log('[JSON推送] API 返回:', result);
                    webviewPanel.webview.postMessage({ type: 'pushSuccess', result });
                } catch (err: any) {
                    console.error('[JSON推送] 异常:', err);
                    webviewPanel.webview.postMessage({ type: 'pushError', message: err?.message || '推送失败' });
                }
            }
            if (msg?.type === 'openTextEditor') {
                await vscode.commands.executeCommand('jsonEditor.openWithFile', filePath);
            }
        });
    }

    private async saveFile(filePath: string, jsonData: { headers?: string[], rows?: string[][] }): Promise<void> {
        if (!jsonData) throw new Error('没有数据可保存');
...
if(msg.type==='jsonData'){try{var decoder=new TextDecoder('utf-8');var uint8Array=new Uint8Array(msg.data);var decoded=decoder.decode(uint8Array);S.data=JSON.parse(decoded);}catch(ex){S.data={headers:[],rows:[]};}S.sel.clear();S.cell=null;S.clip=null;S.mods.clear();S.hist=[];S.hIdx=-1;saveHist();bindEv();updUI();reRender();var loading=document.getElementById('loading');if(loading)loading.style.display='none';}
...
function jsonValue(v){v=String(v||'');if(v.includes(',')||v.includes('"')||v.includes('\\n')||v.includes('\\r')){return'"'+v.replace(/"/g,'""')+'"';}return v;}
function openTextEditor(){var lines=[];if(S.data.headers)lines.push(S.data.headers.map(jsonValue).join(','));S.data.rows.forEach(function(row){lines.push(row.map(jsonValue).join(','));});var csvText=lines.join('\\n');S.vscode.postMessage({type:'openTextEditor',data:csvText});}
