...
async function openWithYamlEditor(uri) {
    const fsPath = uri.fsPath;
    if (processingFiles.has(fsPath))
        return;
    processingFiles.add(fsPath);
...
async function openWithJsonEditor(uri) {
    const fsPath = uri.fsPath;
    if (processingFiles.has(fsPath))
        return;
    processingFiles.add(fsPath);
...
function activate(context) {
    ...
    // 注册自定义编辑器
    context.subscriptions.push(vscode.window.registerCustomEditorProvider('csvEditor.testCase', csvEditorProvider), vscode.window.registerCustomEditorProvider('yamlEditor.testCase', yamlEditorProvider), vscode.window.registerCustomEditorProvider('jsonEditor.testCase', jsonEditorProvider));
    // 监听标签页变化
    context.subscriptions.push(vscode.window.tabGroups.onDidChangeTabs(async (e) => {
        ...
        for (const tab of e.opened) {
            ...
            if (uri) {
                ...
                else if ((0, YamlDocumentProvider_1.isQualifiedYamlFile)(uri)) {
                    if (isUserOpenedAsText(uri)) {
                ...
                else if ((0, JsonDocumentProvider_1.isQualifiedJsonFile)(uri)) {
                    if (isUserOpenedAsText(uri)) {
    ...
    }), vscode.commands.registerCommand('yamlEditor.openWithFile', async (filePath) => {
        if (filePath) {
            // 标记为用户主动用 TextEditor 打开
            markUserOpenedAsText(filePath);
            const uri = vscode.Uri.file(filePath);
    ...
    }), vscode.commands.registerCommand('jsonEditor.openWithFile', async (filePath) => {
        if (filePath) {
            // 标记为用户主动用 TextEditor 打开
            markUserOpenedAsText(filePath);
            const uri = vscode.Uri.file(filePath);
