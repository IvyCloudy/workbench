/**
 * pointCaseLinker · pointId 为空字符串时仍能按路径匹配(type=2)
 * ----------------------------------------------------------------------------
 * 验证 buildIndex 对 pointId 为空字符串的要点不再整体跳过：
 *   只要有合法 pointPath,就纳入 byPath 索引,从而 matchCore 可凭路径兜底命中(type=2)。
...
describe('pointCaseLinker · pointId 为空字符串 → 仍能按路径匹配(type=2)', () => {
    beforeAll(() => fs.mkdirSync(TMP, { recursive: true }));
    afterAll(() => { try { fs.rmSync(TMP, { recursive: true, force: true }); } catch {} });

    it('pointId 为 "" 且 parent_id 不为空 → 凭 pointPath 命中 type=2', async () => {
        ...
        expect(r.byPoint[key][0].type).toBe(2);   // 按路径兜底命中
    });
    ...
    it('pointId 为空不进 byId(无编号),但仍进 byPath(可 path 匹配)', async () => {
        ...
        expect(r.byPoint['_账号登录']?.[0].type).toBe(2);
        // 有编号要点凭 path 命中(type=2)
        expect(r.byPoint['LGN-002_密码错误']).toBeDefined();
        expect(r.byPoint['LGN-002_密码错误'][0].type).toBe(2);
    });
