...
// 解析YAML文件数据
function parseYamlData(filePath: string): { headers: string[], rows: string[][], cols: { [key: string]: { width: number } } } {
    try {
...
// YAML 文档内容提供者
export class YamlDocumentContentProvider implements vscode.TextDocumentContentProvider {
    ...
    private buildYamlHtml(headers: string[], rows: string[][], nonce: string): string {
...
export class YamlEditorProvider implements vscode.CustomEditorProvider {
    ...
    private async saveFile(filePath: string, yamlData: { headers?: string[], rows?: string[][] }): Promise<void> {
        if (!yamlData) throw new Error('没有数据可保存');
