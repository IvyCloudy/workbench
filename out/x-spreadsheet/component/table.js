...
const alphabet_1 = require("../core/alphabet");
const font_1 = require("../core/font");
const cell_1 = __importDefault(require("../core/cell"));
...
const tableGridStyle = {
    fillStyle: '#fff',
    lineWidth: draw_1.thinLineWidth,
    strokeStyle: '#e6e6e6',
};
function tableFixedHeaderStyle() {
    return {
        textAlign: 'center',
        textBaseline: 'middle',
        font: `500 ${(0, draw_1.npx)(12)}px Source Sans Pro`,
        fillStyle: '#585757',
        lineWidth: (0, draw_1.thinLineWidth)(),
        strokeStyle: '#e6e6e6',
    };
}
function getDrawBox(data, rindex, cindex, yoffset = 0) {
    const { left, top, width, height, } = data.cellRect(rindex, cindex);
    return new draw_1.DrawBox(left, top + yoffset, width, height, cellPaddingWidth);
}
/*
...
function renderCell(draw, data, rindex, cindex, yoffset = 0) {
    ...
    draw.rect(dbox, () => {
        ...
        if (!data.settings.evalPaused) {
            cellText = cell_1.default.render(cell.text || '', formula_1.formulam, (y, x) => (data.getCellTextOrDefault(x, y)));
        }
        else {
            cellText = cell.text || '';
        }
        if (style.format) {
            ...
            cellText = format_1.formatm[style.format].render(cellText);
        }
        const font = Object.assign({}, style.font);
        font.size = (0, font_1.getFontSizePxByPt)(font.size);
...
function renderFixedHeaders(type, viewRange, w, h, tx, ty) {
    ...
    if (type === 'all' || type === 'top') {
        data.colEach(viewRange.sci, viewRange.eci, (i, x1, colWidth) => {
            ...
            draw.fillText((0, alphabet_1.stringAt)(ii), x + (colWidth / 2), h / 2);
...
class Table {
    constructor(el, data) {
        this.el = el;
        this.draw = new draw_1.Draw(el, data.viewWidth(), data.viewHeight());
        this.data = data;
    }
    resetData(data) {
        this.data = data;
