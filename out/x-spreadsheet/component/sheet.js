...
const index_1 = __importDefault(require("./toolbar/index"));
const modal_validation_1 = __importDefault(require("./modal_validation"));
const sort_filter_1 = __importDefault(require("./sort_filter"));
...
function overlayerMousemove(evt) {
    ...
    if (evt.buttons !== 0)
        return;
    if (evt.target.className === `${config_1.cssPrefix}-resizer-hover`)
        return;
...
function paste(what, evt) {
    ...
    else if (data.paste(what, msg => (0, message_1.xtoast)('Tip', msg))) {
...
function overlayerMousedown(evt) {
    ...
    const { offsetX, offsetY } = evt;
    const isAutofillEl = evt.target.className === `${config_1.cssPrefix}-selector-corner`;
    ...
    if (!evt.shiftKey) {
        ...
        // mouse move up
        (0, event_1.mouseMoveUp)(window, (e) => {
            ...
            if (isAutofillEl && selector.arange && data.settings.mode !== 'read') {
                if (data.autofill(selector.arange, 'all', msg => (0, message_1.xtoast)('Tip', msg))) {
                    table.render();
                }
            }
...
function sheetInitEvents() {
    ...
    // slide on mobile
    (0, event_1.bindTouch)(overlayerEl.el, {
        move: (direction, d) => {
    ...
    (0, event_1.bind)(window, 'click', (evt) => {
        this.focusing = overlayerEl.contains(evt.target);
    });
    (0, event_1.bind)(window, 'paste', (evt) => {
        if (!this.focusing)
            return;
        paste.call(this, 'all', evt);
        evt.preventDefault();
    });
    (0, event_1.bind)(window, 'copy', (evt) => {
        if (!this.focusing)
            return;
        copy.call(this, evt);
        evt.preventDefault();
    ...
    // for selector
    (0, event_1.bind)(window, 'keydown', (evt) => {
        if (!this.focusing)
            return;
...
class Sheet {
    constructor(targetEl, data) {
        this.eventMap = (0, event_1.createEventEmitter)();
        const { view, showToolbar, showContextmenu } = data.settings;
        this.el = (0, element_1.h)('div', `${config_1.cssPrefix}-sheet`);
        this.toolbar = new index_1.default(data, view.width, !showToolbar);
        this.print = new print_1.default(data);
        targetEl.children(this.toolbar.el, this.el, this.print.el);
        this.data = data;
        // table
        this.tableEl = (0, element_1.h)('canvas', `${config_1.cssPrefix}-table`);
        // resizer
        this.rowResizer = new resizer_1.default(false, data.rows.height);
        this.colResizer = new resizer_1.default(true, data.cols.minWidth);
        // scrollbar
        this.verticalScrollbar = new scrollbar_1.default(true);
        this.horizontalScrollbar = new scrollbar_1.default(false);
        // editor
        this.editor = new editor_1.default(formula_1.formulas, () => this.getTableOffset(), data.rows.height);
        // data validation
        this.modalValidation = new modal_validation_1.default();
        // contextMenu
        this.contextMenu = new contextmenu_1.default(() => this.getRect(), !showContextmenu);
        // selector
        this.selector = new selector_1.default(data);
        this.overlayerCEl = (0, element_1.h)('div', `${config_1.cssPrefix}-overlayer-content`)
            .children(this.editor.el, this.selector.el);
        this.overlayerEl = (0, element_1.h)('div', `${config_1.cssPrefix}-overlayer`)
            .child(this.overlayerCEl);
        // sortFilter
        this.sortFilter = new sort_filter_1.default();
        // root element
        ...
        // table
        this.table = new table_1.default(this.tableEl.el, data);
