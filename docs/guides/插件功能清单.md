...
#### 单元格 / 行 / 列操作
- **编辑**：双击进入 textarea，Enter 提交、Esc 取消；多行内容或拖高行后采用 `position:fixed` 浮窗编辑，Alt/Option+Enter 插入换行，Ctrl/Cmd+Enter 提交
- **受保护列**：`testcase_id` + 其他配置为受保护的列只读，禁止编辑 / 删除 / 重命名
- **表头中英映射**：仅负责展示，不会写回源文件
...
#### 视图与体验
- 列宽拖拽调整、行高拖拽调整、双击分割线自适应
- 双击行号格恢复默认行高；多行内容默认省略，双击可展开查看全部内容
- 顶部搜索框（关键字过滤行）
- **Excel 风格列筛选**（漏斗按钮，支持多列联动统计、值列表勾选）
- **五类互斥筛选**：仅看失败 / 仅看已修改 / 仅看新增 / 仅看已删除 / 仅看已标记
- **查找/替换面板**（`Ctrl/Cmd+F`，支持上一个/下一个、当前替换、全部替换、高亮）
- **撤销 / 重做**（`Ctrl/Cmd+Z` / `Ctrl/Cmd+Y` / `Ctrl/Cmd+Shift+Z`，macOS `Cmd+Y` 已修复），变更、插入、删除、贴上、标记皆纳入历史
- **UI 状态按文件隔离**：列宽 / 行高 / 展开状态 / 筛选 / 搜索 / 滚动位置都按文件路径隔离持久化
...
4. **结果分类**：`type=1` 成功（写回 `testCaseNo`） / `type=2` 失败（记录原因 + 时间戳）
5. **回写**：成功项按 `testcase_id` 把后端返回的 `testCaseNo` 写回原文件
6. **快照基线刷新**：推送后全量更新基线，高亮清理逻辑已简化
7. **结果弹窗**：通过对应 webview `pushResult` 消息展示
...
| 列名 | 写入时机 | 作用 |
|---|---|---|
| `testcase_id` | 第一次打开文件 → `ensureTrackingColumns` 自动生成 `MA` + crypto hex 并立即落盘 | 行的稳定唯一 ID，推送匹配回写、失败持久化键 |
| `testCaseNo` | 推送成功后 → `applyTestCaseNos` 按 `testcase_id` 回写 | 后端返回的案例编号 |

- `testcase_id` 列固定在最前；`testCaseNo` 紧随其后
- YAML / JSON 同步写回 `sourceData`，避免 save 重建嵌套时丢失
- **受保护列**机制：除 `testcase_id` 外，可配置额外受保护列（禁止删除 / 重命名 / 编辑）
...
3. **任务身份唯一来源**：`task-bindings.json`（路径推导逻辑已移除）
4. **未绑定不推送**：保护后端数据
5. **追踪列系统**：`testcase_id`（`MA`+crypto hex）自动生成、禁止编辑、固定首列；可额外配置受保护列
6. **多 Panel 隔离**：每个 Tab 独享 `EditorSession`，状态绝不共享；UI 状态按文件路径隔离持久化
