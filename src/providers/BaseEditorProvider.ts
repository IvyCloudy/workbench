...
export class HttpFetchPushStrategy implements PushStrategy {
    async push(data: any, _filePath: string, webviewPanel: vscode.WebviewPanel): Promise<void> {
...
export abstract class BaseDocumentContentProvider implements vscode.TextDocumentContentProvider {
    private onDidChangeEmitter = new vscode.EventEmitter<vscode.Uri>();
    private dataCache: Map<string, TableData> = new Map();
    ...
    provideTextDocumentContent(uri: vscode.Uri): string {
        ...
        try {
            ...
            content = '<div style="padding:40px;text-align:center;color:#e34d59;">读取失败: ' + escapeHtml(e.message) + '</div>';
        }
    ...
    /** 获取实际文件路径 */
    protected abstract getFilePath(uri: vscode.Uri): string;

    /** 获取预览URI协议 */
    protected abstract getPreviewScheme(): string;

    /** 解析文件数据 */
    protected abstract parseData(filePath: string): TableData;

    /** 获取表格HTML */
    protected buildTableHtml(headers: string[], rows: string[][]): string {
        ...
        html += '<script nonce="' + getNonce() + '">window.csvData=' + jsonData + ';Object.assign(csvData,window.csvData);</script>';
        return html;
    }

    /** 获取HTML包装器 */
    protected getHtmlWrapper(content: string, nonce: string): string {
        return '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' + PREVIEW_STYLES + '</style></head><body>' +
            '<div class="toolbar">' +
            ...
            '<script nonce="' + nonce + '">' + PREVIEW_SCRIPTS + '</script></body></html>';
    }
}
