...
const auto_filter_1 = __importDefault(require("./auto_filter"));
...
function canPaste(src, dst, error = () => { }) {
    ...
    if (merges.intersects(cellRange)) {
        error((0, locale_1.t)('error.pasteForMergedCell'));
        return false;
    }
    return true;
}
function copyPaste(srcCellRange, dstCellRange, what, autofill = false) {
    ...
    rows.copyPaste(srcCellRange, dstCellRange, what, autofill, (ri, ci, cell) => {
        if (cell && cell.merge) {
            ...
            if (rn <= 0 && cn <= 0)
                return;
            merges.add(new cell_range_1.CellRange(ri, ci, ri + rn, ci + cn));
        }
    });
}
...
function setStyleBorder(ri, ci, bss) {
    ...
    if (cell.style !== undefined) {
        cstyle = helper_1.default.cloneDeep(styles[cell.style]);
    }
    cstyle = helper_1.default.merge(cstyle, { border: bss });
    ...
    if (mode === 'outside' && !multiple) {
        ...
        selector.range.each((ri, ci) => {
            ...
            if (cell && cell.style !== undefined) {
                const ns = helper_1.default.cloneDeep(styles[cell.style]);
                delete ns.border;
...
function getCellColByX(x, scrollOffsetx) {
    ...
    if (fsw + cols.indexWidth < x)
        inits -= scrollOffsetx;
    const [ci, left, width] = helper_1.default.rangeReduceIf(0, cols.len, inits, cols.indexWidth, x, i => cols.getWidth(i));
    if (left <= 0) {
...
class DataProxy {
    constructor(name, settings) {
        this.settings = helper_1.default.merge(defaultSettings, settings || {});
        // save data begin
        this.name = name || 'sheet';
        this.freeze = [0, 0];
        this.styles = []; // Array<Style>
        this.merges = new merge_1.Merges(); // [CellRange, ...]
        this.rows = new row_1.Rows(this.settings.row);
        this.cols = new col_1.Cols(this.settings.col);
        this.validations = new validation_1.Validations();
        this.hyperlinks = {};
        this.comments = {};
        // save data end
        // don't save object
        ...
        this.autoFilter = new auto_filter_1.default();
        this.change = () => { };
    ...
    calSelectedRangeByEnd(ri, ci) {
        ...
        if (nci > cci)
            ...
            [sci, eci] = [nci, cci];
        selector.range = merges.union(new cell_range_1.CellRange(sri, sci, eri, eci));
    ...
    calSelectedRangeByStart(ri, ci) {
        ...
        if (cellRange === null) {
            cellRange = new cell_range_1.CellRange(ri, ci, ri, ci);
            if (ri === -1) {
                cellRange.sri = 0;
                ...
                if (selector.multiple()) {
                    ...
                    if (rn > 1) {
                        for (let i = sci; i <= eci; i += 1) {
                            ...
                            cell.text = `=${value}(${(0, alphabet_1.xy2expr)(i, sri)}:${(0, alphabet_1.xy2expr)(i, eri)})`;
                        }
                    }
                    else if (cn > 1) {
                        const cell = rows.getCellOrNew(ri, eci + 1);
                        cell.text = `=${value}(${(0, alphabet_1.xy2expr)(sci, ri)}:${(0, alphabet_1.xy2expr)(eci, ri)})`;
                    }
                }
                else {
                    const cell = rows.getCellOrNew(ri, ci);
        ...
        this.changeData(() => {
            ...
            if (type === 'row') {
                ...
                Object.keys(cols._).forEach((colIndex) => {
                    ...
                    if (col >= sci) {
                        if (col > eci)
                            cols._[col - (eci - sci + 1)] = cols._[col];
                        delete cols._[col];
                    }
                });
            }
    ...
    scrollx(x, cb) {
        ...
        const [, fci] = freeze;
        const [ci, left, width,] = helper_1.default.rangeReduceIf(fci, cols.len, 0, 0, x, i => cols.getWidth(i));
    ...
    scrolly(y, cb) {
        ...
        const [fri] = freeze;
        const [ri, top, height,] = helper_1.default.rangeReduceIf(fri, rows.len, 0, 0, y, i => rows.getHeight(i));
        let y1 = top;
        if (y > 0)
            y1 += height;
    ...
    getCellStyleOrDefault(ri, ci) {
        ...
        return helper_1.default.merge(this.defaultStyle(), cellStyle);
    }
    ...
    freezeViewRange() {
        const [ri, ci] = this.freeze;
        return new cell_range_1.CellRange(0, 0, ri - 1, ci - 1, this.freezeTotalWidth(), this.freezeTotalHeight());
    }
    contentRange() {
        const { rows, cols } = this;
        ...
        return new cell_range_1.CellRange(0, 0, ri, ci, w, h);
    }
    ...
    viewRange() {
        ...
        return new cell_range_1.CellRange(ri, ci, eri, eci, x, y);
    }
    ...
    addStyle(nstyle) {
        ...
        for (let i = 0; i < styles.length; i += 1) {
            ...
            if (helper_1.default.equals(style, nstyle))
                return i;
        }
        styles.push(nstyle);
    ...
    setData(d) {
        Object.keys(d).forEach((property) => {
            if (property === 'merges' || property === 'rows'
                ...
                const [x, y] = (0, alphabet_1.expr2xy)(d[property]);
                this.freeze = [y, x];
            }
    ...
    getData() {
        ...
        return {
            name,
            freeze: (0, alphabet_1.xy2expr)(freeze[1], freeze[0]),
            styles,
